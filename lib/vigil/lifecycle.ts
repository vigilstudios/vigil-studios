...
export function describeDomainStatus(status: DomainStatus): CustomerStatus {
  switch (status) {
    case "connected":
      return { label: "Domain Connected", tone: "good" };
    case "verifying":
      return { label: "Checking DNS", tone: "info", hint: "Changes can take up to 48 hours to propagate." };
    case "pending":
