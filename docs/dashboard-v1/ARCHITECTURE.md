...
| D9 | Lifecycle states are Postgres enums with an app-level transition table (`lib/vigil/lifecycle.ts`) **and** a database trigger that writes `audit_events` on every status change of projects, websites, domains, subscriptions, jobs. | Auditable state transitions regardless of which code path made the change. |
